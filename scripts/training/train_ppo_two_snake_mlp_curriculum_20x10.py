...
20x10 Rectangular Grid Version

5-stage curriculum implementation extending the base PPO two-snake trainer.
Trained on a 20x10 rectangular grid instead of square grids.
...
Grid size: 20 (width) x 10 (height) = 200 cells
"""
...
class CurriculumPPOTrainer(TwoSnakePPOTrainer):
    ...
    def __init__(self, config: PPOConfig):
        # Initialize parent WITHOUT calling super().__init__() first
        # We need to manually set up components to use 20x10 grid
        ...
        # Create environment with 20x10 rectangular grid
        self.env = VectorizedTwoSnakeEnv(
            num_envs=config.num_envs,
            grid_width=20,  # MODIFIED: Rectangular grid
            grid_height=10,  # MODIFIED: Rectangular grid
            max_steps=config.max_steps,
            target_food=config.target_food,
            device=self.device
        )

        # Now call parent's init methods for agent setup
        # Skip parent __init__ and manually set up what we need
        self.obs_dim = 35  # Feature vector size
        self.action_dim = 3  # STRAIGHT, RIGHT, LEFT
        ...
        # Training state
        self.total_steps = 0
        self.episode_rewards1 = torch.zeros(config.num_envs, device=self.device)
        self.episode_rewards2 = torch.zeros(config.num_envs, device=self.device)
        self.episode_lengths = torch.zeros(config.num_envs, device=self.device)
    ...
    def train(self):
        ...
        print("\n" + "="*70, flush=True)
        print("PPO TWO-SNAKE CURRICULUM TRAINING (MLP) - 20x10 Grid", flush=True)
        print("="*70, flush=True)
        print(f"Device: {self.device}", flush=True)
        print(f"Num environments: {self.config.num_envs}", flush=True)
        print(f"Grid size: 20x10 (rectangular grid)", flush=True)
        print(f"Total cells: 200", flush=True)
...
def main():
    parser = argparse.ArgumentParser(
        description='PPO Two-Snake Curriculum Training (MLP) - 20x10 Rectangular Grid',
    ...
    parser.add_argument('--save-dir', type=str,
                        default='results/weights/ppo_two_snake_mlp_curriculum_20x10',
